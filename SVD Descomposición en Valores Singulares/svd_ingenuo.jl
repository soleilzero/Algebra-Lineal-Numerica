...
begin
	...
	for n in ns
	    ...
	    # --- SVD Julia ---
	    t_julia = @belapsed svd($A)
	    err_julia = norm(A - reconstruct_from_svd(svd(A)))
	
	    # --- SVD propia ---
	    t_naive = @belapsed naiveSVD_classic_($A)
	    err_naive = norm(A - reconstruct_from_svd(naiveSVD_classic_(A)))
	
	    # Guardar
	    push!(tiempos_naive, t_naive)
	    push!(tiempos_julia, t_julia)
	    push!(errores_naive, err_naive)
	    push!(errores_julia, err_julia)
	end
end

# ╔═╡ 52d02aa0-c9b7-4f62-9fed-48630114297f
md"### Comparación de tiempo"

# ╔═╡ cdecd567-bce8-4e40-af99-1687a8dcef80
plot(ns, [tiempos_naive, tiempos_julia], 
    label=["SVD propio" "SVD Julia"],
    title="Tiempo de ejecución",
    xlabel="Tamaño matriz (n x n)", 
    ylabel="Tiempo (s)", 
    lw=2, marker=:circle)
...
# ╔═╡ 8273af97-3aa6-4048-b9c9-65c9e6525f9d
display(
	DataFrame(
	    n = collect(ns),
	    Ingenuo = tiempos_naive,
...
# ╔═╡ 0323ac35-ccb6-41b3-8bb1-d9734eb1f882
plot(ns, [errores_naive, errores_julia], 
    label=["Error propio" "Error Julia"],
    title="Error de reconstrucción ||A - UΣVᵀ||",
    xlabel="Tamaño matriz (n x n)", 
    ylabel="Error (norma 2)", 
    lw=2, marker=:circle, yscale=:log10)

# ╔═╡ 44808845-c3a0-4d92-b4b9-6e7e4fac3641
md"""
### Resultados
...
manifest_format = "2.0"
project_hash = "0292a76c50042a4aa74b10ed84dae3d280e9240f"
