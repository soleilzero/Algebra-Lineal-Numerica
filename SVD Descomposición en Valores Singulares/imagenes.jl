...
**Estrategia:**
Resolver el problema mediante un **método iterativo** basado en SVD, que reconstruye las regiones faltantes aproximando la matriz completa al rango deseado $k$.
...
- `threshold::Float64=0.95`: Umbral para detectar zonas faltantes.
    - Píxeles con valor >= threshold se consideran dañados o faltantes.

Retorna `M::Array{Float64}`: Máscara binaria donde 1.0 indica píxel conocido y 0.0 indica píxel faltante.
"""
function generate_mask(img, threshold=0.95)
    return map(x -> x < threshold ? 1.0 : 0.0, img)
end
